...
class SoapScoreInterpreter {
  ...
  _computeDurationFromEventToLocation(event, targetBar, targetBeat) {
    ...
    if (event.tempo.curve === null) {
      ...
      const numBarNormalized = (targetBar - (event.bar + 1))
        + (1 - (event.beat - 1) / numBasisInBar)
        + ((targetBeat - 1) / numBasisInBar);
    ...
    // handle curve
      ...
      while (bar < targetBar || (bar === targetBar && beat < targetBeat)) {
        ...
        // @important: we want to stay in the simple signature.upper coordinates
        // here, so we ignore additive signatures
        beat += 1;
  ...
  getLocationAtPosition(targetPosition) {
    ...
    // bars are defined by absolute duration, just find the closest bar
      ...
      if (event.tempo.curve === null) {
        ...
        // number of tempo basis noramalized to bar
        const numBarNormalized = numBasis / numBasisInBar;
        //
        const numBars = Math.floor(numBarNormalized);

        const numBeatsFull = numBarNormalized * numBasisInBar;
        const numBeats = numBeatsFull % numBasisInBar;

        let bar = event.bar + numBars;
        let beat = event.beat + numBeats;

        return this._wrapAndParseAdditive(event.signature, bar, beat);
      } else {
        ...
        while (delta > 0) {
          ...
          if (beatDuration <= delta) {
            ...
            const location = this.getNextLocation(bar, beat, true);
  ...
  _wrapAndParseAdditive(signature, bar, beat) {
    // clean floating point error of beats
    if (Math.abs(Math.round(beat) - beat) < 1e-3) {
      beat = Math.round(beat);
    }

    // wrap bar, beat in next measure if needed
    if (beat > signature.upper) {
      bar += 1;
      beat -= signature.upper;
    }

    // for additive signatures we have another layer of complexity
    // i.e. [3+2+2/8] will have signature.upper == 7 but we have only 3 beats
    // until this point beat is counted according to 7, not 3
    if (signature.additive.length > 0) {
      beat = beat - 1;
      let targetBeat = 1;

      for (let i = 0; i < signature.additive.length; i++) {
        let subBeat = signature.additive[i];

        // we can add this sub beat to resulting beat
        if (beat - subBeat >= 0) {
          targetBeat += 1;
          beat -= subBeat;
          // we have reached the right beat
          if (beat === 0) {
            beat = targetBeat;
            break;
          }
        } else {
          // get remaining portion according to subBeat
          targetBeat += beat / subBeat;
          // override beat with result and break
          beat = targetBeat;
          break;
        }
      }
  ...
  getLocationInfos(bar, beat) {
    ...
    if (signature.additive.length > 0) {
      const localBasis = TimeSignature.get([signature.additive[Math.round(beat - 1)], signature.lower]);
  ...
  getNextLocation(bar, beat, ignoreAdditive = false) {
    ...
    if (currentEvent.duration) {
      ...
      //
      if (currentSignature.additive.length > 0 && !ignoreAdditive) {
